...
import { FinancingPlanForm } from "../financing/FinancingPlanForm";
...
const statusColors = {
  ...
  'Convertido a Plan': 'bg-purple-100 text-purple-800',
};
...
export function BudgetsModule() {
  ...
  const handleCreateFinancing = (budget: Budget) => {
    if (budget.status !== 'Aprobado') {
      toast.error('Solo se pueden crear planes de financiamiento para presupuestos aprobados');
      return;
    }
    setSelectedBudget(budget);
  ...
  const handleFinancingSaved = () => {
    ...
    setActiveTab("list");
    setSelectedBudget(null);
    toast.success('Plan de financiamiento creado exitosamente');
  };
  ...
  const handleConvertToTreatment = async (budgetId: string) => {
    if (!confirm('¿Estás seguro de que quieres convertir este presupuesto a un plan de tratamiento? Esto también generará una orden de tratamiento que el paciente deberá firmar.')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        fetchBudgets();
        toast.success(result.message || 'Presupuesto convertido exitosamente. Se ha generado una orden de tratamiento pendiente de firma.');
      } else {
        toast.error(result.error || 'Error al convertir el presupuesto');
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="financing" disabled={!selectedBudget || selectedBudget.status !== 'Aprobado'}>Plan de Financiamiento</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="list" className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Convertido a Plan">Convertido a Plan</SelectItem>
                  </SelectContent>
                </Select>
          ...
          <div className="grid gap-4">
            {filteredBudgets.map((budget) => {
              const StatusIcon = statusIcons[budget.status as keyof typeof statusIcons] || statusIcons.Borrador;
              return (
                      ...
                      <div className="flex gap-2 ml-4">
                        ...
                        {budget.status === 'Aprobado' && (
                          <Button
                            ...
                            onClick={() => handleConvertToTreatment(budget.id)}
                          >
        ...
        <TabsContent value="detail">
          {selectedBudget && (
            <BudgetDetail 
              ...
              onFinancing={handleCreateFinancing}
            />
          )}
        </TabsContent>

        <TabsContent value="financing">
          {selectedBudget && showFinancingForm && (
            <div className="text-center py-12">
              <p className="text-gray-600 mb-4">Plan de financiamiento para: {selectedBudget.title}</p>
              <p className="text-sm text-gray-500 mb-6">Total del presupuesto: {formatCurrency(selectedBudget.total)}</p>
              <Button 
                onClick={() => {
                  setShowFinancingForm(false);
                  setActiveTab("list");
