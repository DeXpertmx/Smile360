...
export default function PatientPortalPage() {
  ...
  const fetchPatientData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Establecer todos los datos del paciente
        if (data.patient) {
        ...
        // Establecer órdenes de tratamiento
        setTreatmentOrders(data.treatmentOrders || []);
        ...
        // Mock documents for now (can be replaced with real data later)
        const mockDocs = data.treatmentOrders?.map((order: TreatmentOrder) => ({
          id: order.id,
          name: `Orden de Tratamiento - ${order.procedureType}`,
          type: 'treatment-order',
          date: order.createdDate,
          downloadUrl: `/api/treatment-orders/${order.id}/pdf`,
          status: order.status,
  ...
  const handleAppointmentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchPatientData(); // Wait for the fetch to complete
      } else {
  ...
  // Funciones para órdenes de tratamiento
  const handleSignOrder = (orderId: string) => {
    const order = treatmentOrders.find(o => o.id === orderId);
    if (order) {
      router.push(`/portal/sign/${orderId}`);
    }
  };

  const getOrderStatusBadge = (status: string) => {
    const config: { [key: string]: { color: string; label: string; icon: any } } = {
      'Pendiente': { color: 'bg-yellow-100 text-yellow-800', label: 'Pendiente de Firma', icon: Clock },
      'Firmada': { color: 'bg-green-100 text-green-800', label: 'Firmada', icon: CheckCircle },
      'Rechazada': { color: 'bg-red-100 text-red-800', label: 'Rechazada', icon: AlertCircle },
      'En_Proceso': { color: 'bg-blue-100 text-blue-800', label: 'En Proceso', icon: Activity },
      'Completada': { color: 'bg-green-100 text-green-800', label: 'Completada', icon: CheckCircle },
    };
    
    const statusInfo = config[status] || { color: 'bg-gray-100 text-gray-800', label: status, icon: FileText };
  ...
  // Calculate progress and stats usando los datos reales de la API
  ...
  const treatmentProgress = stats.progressPercentage || 0;
  const pendingOrders = stats.pendingOrders || 0;
  const signedOrders = stats.signedOrders || 0;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs defaultValue="dashboard" className="space-y-6">
          <TabsList className="grid w-full grid-cols-6">
            ...
            <TabsTrigger value="treatment-orders">Tratamientos</TabsTrigger>
          ...
          <TabsContent value="treatment-orders">
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <CardDescription>
                      Órdenes de tratamiento y progreso de tus procedimientos dentales
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                {/* Resumen rápido de tratamientos */}
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold text-green-600">{signedOrders}</p>
                        </div>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold text-blue-600">{treatmentProgress}%</p>
                        </div>
                ...
                {/* Lista de órdenes de tratamiento */}
                <div className="space-y-4">
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-3">
                              ...
                              <div>
                                <h3 className="font-semibold text-lg">{order.procedureType}</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                              <div>
                                ...
                                <p className="font-medium">Dr(a). {order.doctor.firstName} {order.doctor.lastName}</p>
                              </div>
                              <div>
                                <span className="text-sm text-gray-500">Fecha de Creación:</span>
                                <p className="font-medium">
                                  {new Date(order.createdDate).toLocaleDateString('es-ES')}
                                </p>
                              </div>
                              <div>
                            ...
                            {order.treatmentDetails && (
                              <div className="mb-4">
                                <span className="text-sm text-gray-500">Plan de Tratamiento:</span>
                                <p className="text-sm mt-1 p-3 bg-gray-50 rounded-lg">
                                  {order.treatmentDetails.substring(0, 150)}
                                  {order.treatmentDetails.length > 150 && '...'}
                                </p>
                              </div>
                            )}
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex gap-2">
                                ...
                                {order.status === 'Firmada' && order.patientSignature && (
                                  <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                                    <CheckCircle className="w-3 h-3 mr-1" />
                                    Firmado el {order.signatureDate && new Date(order.signatureDate).toLocaleDateString('es-ES')}
                                  </Badge>
                                )}
                              </div>
                            </div>
                  ...
                  {treatmentOrders.length === 0 && (
                    <div className="text-center py-12 text-gray-500">
                      ...
                      <h3 className="text-lg font-medium mb-2">No tienes órdenes de tratamiento</h3>
                      <p className="text-sm">
                        Las órdenes de tratamiento aparecerán aquí cuando tu dentista las cree.
                      </p>
                      <p className="text-sm mt-1">
                        Podrás revisar y firmar digitalmente cada tratamiento desde esta sección.
                      </p>
                    </div>
                  )}
                </div>
