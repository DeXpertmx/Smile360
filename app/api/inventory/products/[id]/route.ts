  ...
  try {
    ...
    const product = await prisma.product.update({
      ...
      data: {
        ...
        totalStock: parseInt(data.totalStock?.toString() || '0'),
    ...
    if (product.totalStock !== existingProduct.totalStock) {
      const difference = product.totalStock - existingProduct.totalStock;
      ...
      await prisma.inventoryTransaction.create({
        data: {
          ...
          stockBefore: existingProduct.totalStock,
...
async function checkAndUpdateAlerts(product: any) {
  try {
    ...
    if (product.totalStock <= product.minStock) {
      const alertType = product.totalStock === 0 ? 'agotado' : 'stock_bajo';
      const priority = product.totalStock === 0 ? 'Alta' : 'Normal';
        ...
        await prisma.inventoryAlert.create({
          data: {
            ...
            message: product.totalStock === 0 
              ...
              : `El producto ${product.name} tiene stock bajo (${product.totalStock} unidades)`,
            priority
          }
        });
      }
